...
(async () => {
    try { 
        ...
        await pool.query('CREATE TABLE player (playerId INT UNSIGNED NOT NULL AUTO_INCREMENT, name VARCHAR(255) NOT NULL, birthdate DATE NULL, debut DATE NULL, number INT DEFAULT 0, teamId INT UNSIGNED NOT NULL, rating FLOAT DEFAULT 0, salary INT DEFAULT 0, FOREIGN KEY (teamId) REFERENCES team (teamId) ON DELETE RESTRICT ON UPDATE CASCADE, PRIMARY KEY (playerId)) ENGINE = InnoDB');
        await pool.query('INSERT INTO player VALUES (NULL, "Dick Butkus", "1942-12-09", "1965-09-01", 51, 1, 5.0, 5000000)');
        
        await pool.query('CREATE TABLE coach (coachId INT UNSIGNED NOT NULL AUTO_INCREMENT, name VARCHAR(255) NOT NULL, coachType VARCHAR(255) DEFAULT "", teamId INT UNSIGNED NOT NULL, rating FLOAT DEFAULT 0, salary INT DEFAULT 0, FOREIGN KEY (teamId) REFERENCES team (teamId) ON DELETE RESTRICT ON UPDATE CASCADE, PRIMARY KEY (coachId)) ENGINE = InnoDB');
